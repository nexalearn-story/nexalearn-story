...
# 2yMXr9lEPjKi4D6PYH97DhcNz48_6PKxNtTCsUdSsyhepy3cL
...
# from os.path import exists
# import traceback
# from collections import defaultdict ## NEW ## A helper for grouping
# from flask import (
#     Flask, render_template, request, redirect,
#     flash, url_for, send_from_directory, abort
# )
...
# ## NEW ## Define the categories for your platform
# CATEGORIES = [
#     'Story',
#     'School Lessons',
#     'University Courses',
...
#             conn = get_db_connection()
#             cursor = conn.cursor()
#             ## NEW ## Updated INSERT statement to include category
#             cursor.execute("""
#               INSERT INTO stories
#                 (story_name, author_name, description, cover_image, category)
#               VALUES (%s, %s, %s, %s, %s)
...
#                 # Validate episode inputs
#                 if not title:
#                     raise ValueError(f"Title missing for episode {i}")
#                 if not audio or not allowed_file(audio.filename, ALLOWED_AUDIO_EXT):
#                     raise ValueError(f"Valid audio required for episode {i}")
#                 if not img or not allowed_file(img.filename, ALLOWED_IMAGE_EXT):
#                     raise ValueError(f"Valid image required for episode {i}")
...
#     ## NEW ## Pass the list of categories to the template for the dropdown
#     return render_template('submit.html', categories=CATEGORIES)
...
#     # fetch story
#     ## NEW ## Also fetch category to display on the page if you want
#     cur.execute("""
#       SELECT story_id, story_name, author_name, description, cover_image, category
#       FROM stories
#       WHERE story_id = %s
#     """, (story_id,))
#     story = cur.fetchone()
...
#     # fetch episodes
#     cur.execute("""
#       SELECT episode_number, title, audio_file, image_file
#       FROM episodes
#       WHERE story_id = %s
...
#         conn = get_db_connection()
#         cursor = conn.cursor(dictionary=True)
#         ## NEW ## Fetch category along with other details
#         cursor.execute("""
#           SELECT story_id, story_name, author_name, cover_image, category
#           FROM stories
#           ORDER BY story_id DESC
#         """)
#         all_stories = cursor.fetchall()
        
#         ## NEW ## Group stories by their category
#         stories_by_category = defaultdict(list)
#         for story in all_stories:
#             stories_by_category[story['category']].append(story)

#         ## NEW ## Pass the grouped stories and the category list to the template
#         return render_template(
#             'index.html',
#             stories_by_category=stories_by_category,
#             categories=CATEGORIES # Pass the ordered list of categories
#         )
...
# #             cursor.execute("""
# #               INSERT INTO stories
# #                 (story_name, author_name, description, cover_image)
# #               VALUES (%s, %s, %s, %s)
# #             """, (story, author, desc, cover_fn))
# #             story_id = cursor.lastrowid
...
# #     # fetch story
# #     cur.execute("""
# #       SELECT story_id, story_name, author_name, description, cover_image
# #       FROM stories
# #       WHERE story_id = %s
# #     """, (story_id,))
# #     story = cur.fetchone()
...
# #             writer = csv.writer(file)
# #             if not file_exists:
# #                 writer.writerow(['Name', 'Email', 'Message'])  # Header row
# #             writer.writerow([name, email, message])

# #         flash("Thanks for reaching out! We'll get back to you soon.", "success")
# #         return "<h1>Thanks for reaching out! We'll get back to you soon.", "success</h1>"
...
# #         cursor = conn.cursor(dictionary=True)
# #         cursor.execute("""
# #           SELECT story_id, story_name, author_name, cover_image
# #           FROM stories
# #           ORDER BY story_id DESC
# #         """)
