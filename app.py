...
# --- CONFIGURATION ---
# === EDIT THESE FOUR LINES TO MATCH YOUR HEROKU CONFIG VARS ===
# ===
# === For example, if your variable is BUCKETEER_AMBER_BUCKET_NAME, change the line to:
# === S3_BUCKET = os.environ.get('BUCKETEER_AMBER_BUCKET_NAME')
# ===
...
S3_SECRET_KEY = os.environ.get('BUCKETEER_AWS_SECRET_KEY')
# =============================================================
...
CATEGORIES = [ 'Story', 'School Lessons', 'University Courses', 'Novels', 'Biographies', 'Science & Technology', 'History', 'Self-Help & Motivation', 'Business & Finance', 'Children\'s Stories', 'Poetry', 'Other']
>>>>>>> 47111e46e801dcd55a8a1927384be8dfd0764891
...
def get_s3_client():
    if not all([S3_BUCKET, S3_REGION, S3_ACCESS_KEY_ID, S3_SECRET_KEY]):
        app.logger.error(f"S3 configuration is missing! Check Config Vars. BUCKET={S3_BUCKET}, REGION={S3_REGION}")
        return None
    try:
        s3 = boto3.client(
           "s3",
...
def submit():
    if request.method == 'POST':
        try:
            # This check happens after you fill out the form and click "upload"
            s3 = get_s3_client()
    ...
    # This is for the GET request when you first load the page.
    # The error flash you see happens before you even try to upload.
...
def index():
    try:
        conn = get_db_connection()
        if not conn: return render_template('error.html', message="Could not connect to the database."), 500
        ...
        return render_.template('error.html', message="An unexpected error occurred."), 500
    finally:
...
# # --- CONFIGURATION ---
# # This is the simple, correct way. It reads the default variables
# # that your new add-on has created.
# S3_BUCKET = os.environ.get('BUCKETEER_BUCKET_NAME')
# S3_REGION = os.environ.get('BUCKETEER_AWS_REGION')
# S3_ACCESS_KEY_ID = os.environ.get('BUCKETEER_AWS_ACCESS_KEY_ID')
# S3_SECRET_KEY = os.environ.get('BUCKETEER_AWS_SECRET_KEY')
...
# def get_s3_client():
#     # Using the simplest client, which is the correct way for Heroku.
#     if not all([S3_BUCKET, S3_REGION, S3_ACCESS_KEY_ID, S3_SECRET_KEY]):
#         app.logger.error("S3 configuration is missing! Check Config Vars.")
#         return None
#     try:
#         s3 = boto3.client(
#            "s3",
...
#             flash("An unexpected server error occurred.", 'error')
#             return redirect(request.url)
#     # This is for the GET request to the page
#     return render_template('submit.html', categories=CATEGORIES)
...
# # # --- CONFIGURATION ---
# # # This new logic automatically finds the correct Bucketeer variables.
# # # You do not need to edit this.
# # def get_bucketeer_env_var(var_name):
# #     """Gets the correct Bucketeer env var, checking for color names."""
# #     # Check for the primary add-on name first (e.g., BUCKETEER_MAUVE_...)
# #     # We can find the 'color' prefix by checking other variables.
# #     for key in os.environ:
# #         if key.endswith('_BUCKET_NAME') and key.startswith('BUCKETEER_'):
# #             prefix = key.replace('BUCKET_NAME', '')
# #             value = os.environ.get(f"{prefix}{var_name}")
# #             if value:
# #                 return value
...
# # def get_s3_client():
# #     # Use the simplest possible client. This is the correct way for Heroku.
# #     if not all([S3_BUCKET, S3_REGION, S3_ACCESS_KEY_ID, S3_SECRET_KEY]):
# #         app.logger.error("S3 configuration is missing! Check env vars.")
# #         app.logger.error(f"Vars: BUCKET={S3_BUCKET}, REGION={S3_REGION}, KEY_ID={S3_ACCESS_KEY_ID}") # More logs
# #         return None
# #     try:
# #         s3 = boto3.client(
# #            "s3",
...
# # # from mysql.connector import Error as MySQLError
# # # import boto3
# # # # REMOVED: No longer need Config from botocore.client
# # # from botocore.exceptions import ClientError
...
# # # # --- CONFIGURATION ---
# # # # We use the _MAUVE_ variables as they are the primary ones.
# # # S3_BUCKET = os.environ.get('BUCKETEER_MAUVE_BUCKET_NAME')
# # # S3_REGION = os.environ.get('BUCKETEER_MAUVE_AWS_REGION')
# # # S3_ACCESS_KEY_ID = os.environ.get('BUCKETEER_MAUVE_AWS_ACCESS_KEY_ID')
# # # S3_SECRET_KEY = os.environ.get('BUCKETEER_MAUVE_AWS_SECRET_ACCESS_KEY')
...
# # #     # We are resetting the client to its simplest form.
# # #     # No extra configs. We will let boto3 and the Heroku environment
# # #     # handle the configuration automatically, as intended.
# # #     if not all([S3_BUCKET, S3_REGION, S3_ACCESS_KEY_ID, S3_SECRET_KEY]):
...
# # #            aws_access_key_id=S3_ACCESS_KEY_ID,
# # #            aws_secret_access_key=S3_SECRET_KEY
# # #            # All extra 'config' and 'endpoint_url' parameters have been removed.
# # #         )
# # #         return s3
# # #     except Exception as e:
# # #         app.logger.error(f"S3 client creation error: {e}")
...
# # # # --- ROUTES (No changes are needed in any of the routes) ---
# # # @app.route('/submit', methods=['GET', 'POST'])
# # # def submit():
# # #     # ... No changes ...
# # #     if request.method == 'POST':
...
# # #                 if episode.get('audio_file'):
# # #                     try:
# # #                         episode['audio_url'] = s3.generate_presigned__url('get_object', Params={'Bucket': S3_BUCKET, 'Key': episode['audio_file']}, ExpiresIn=3600)
# # #                     except ClientError as e:
# # #                         app.logger.error(f"Error for audio {episode['audio_file']}: {e}")
# # #                 if episode.get('image_file'):
# # #                     try:
...
# # # # }
# # # # # --- THIS IS THE FIX ---
# # # # # We are now reading the correct '_MAUVE_' variables provided by Heroku.
# # # # # These are the credentials that have the correct permissions.
# # # # S3_BUCKET = os.environ.get('BUCKETEER_MAUVE_BUCKET_NAME')
# # # # S3_REGION = os.environ.get('BUCKETEER_MAUVE_AWS_REGION')
# # # # S3_ACCESS_KEY_ID = os.environ.get('BUCKETEER_MAUVE_AWS_ACCESS_KEY_ID')
# # # # S3_SECRET_KEY = os.environ.get('BUCKETEER_MAUVE_AWS_SECRET_ACCESS_KEY')
...
# # # # # This function is now correct because it receives the correct credentials from the config.
# # # # def get_s3_client():
# # # #     if not all([S3_BUCKET, S3_REGION, S3_ACCESS_KEY_ID, S3_SECRET_KEY]):
# # # #         app.logger.error("S3 configuration is missing!")
# # # #         return None
...
# # # # #         return None
# # # # #     try:
# # # # #         # Explicitly build the regional endpoint URL. This is the key.
# # # # #         endpoint_url = f"https://s3.{S3_REGION}.amazonaws.com"
...
# # # # #            aws_secret_access_key=S3_SECRET_KEY,
# # # # #            config=s3_config,
# # # # #            endpoint_url=endpoint_url # Apply the explicit endpoint URL
# # # # #         )
# # # # #         return s3
# # # # #     except Exception as e:
# # # # #         app.logger.error(f"S3 client creation error: {e}")
...
# # # # # # --- ROUTES ---
# # # # # # No changes are needed in any of your routes. They are all correct.
...
# # # # # #         # 1. Use Signature Version 4 (Best Practice)
# # # # # #         # 2. Set addressing_style to 'path' to prevent redirects
# # # # # #         s3_config = Config(
# # # # # #             signature_version='s3v4',
# # # # # #             s3={'addressing_style': 'path'}
# # # # # #         )
...
# # # # # #            aws_access_key_id=S3_ACCESS_KEY_ID,
# # # # # #            aws_secret_access_key=S3_SECRET_KEY,
# # # # # #            config=s3_config # Apply our new, correct configuration
# # # # # #         )
# # # # # #         return s3
# # # # # #     except Exception as e:
# # # # # #         app.logger.error(f"S3 client creation error: {e}")
...
# # # # # # # Your routes for submit, index, story, contact, etc., are all correct and
# # # # # # # do not need to be changed. They will now work because get_s3_client()
# # # # # # # provides a correctly configured client.
